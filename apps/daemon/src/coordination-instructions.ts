...
Suggest a goal when sustained work would benefit from durable ownership and supervision; do not create an artificial goal for a status question. After the Human agrees to a proposal, retain the relevant request IDs in sourceConversationIds and the agreed objective and constraints. This links the discussion without deleting it. Goal proposals do not authorize execution. Human approval happens through the operator controls; nanasa.request_human_decision cannot approve a proposed goal. Inspect current policy rather than assuming a supervision mode from project instructions.
