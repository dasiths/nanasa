...
export function agentDirectoryEntries(snapshot: PortalSnapshot, config: NanasaConfig) {
  ...
  return snapshot.memberships
    ...
    .map((member) => {
      ...
      const configuredAgents = Object.entries(configuredGroup?.agents ?? {});
      const configuredEntry =
        configuredAgents.find(([, candidate]) => candidate.memberId === member.memberId) ??
        configuredAgents.find(([agentId]) => agentId === member.agentProfileId);
      ...
      return {
        ...
        providerHome:
          integration === undefined
            ? undefined
            : configuredProviderHome(integration, configuredEntry![0]),
